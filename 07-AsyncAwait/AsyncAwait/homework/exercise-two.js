...
async function problemA () {
  ...
  let promises = stanzas.map(stanza =>  readFileAsnc(stanza) )
...
async function problemB () {
  ...
  let promises = filenames.map(stanza =>  readFileAsnc(stanza) )
...
async function problemC () {
  ...
  // }
  // const archivos = [];
  // filenames.forEach((file) => archivos.push(blue(readFileAsnc(file))))

  // await Promise.all(archivos);
  // console.log('done');
  const result = await Promise.all(filenames.map(file => promisifiedReadFile(file)))
  result.forEach((stanza) => blue(stanza))
  console.log('done')
}
